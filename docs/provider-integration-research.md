...
An **accepted** extraction terminates the fallback chain. For global extraction, an abstention, invalid JSON, schema mismatch, missing evidence, or low-confidence result also terminates the chain and is persisted as a non-public review outcome; provider failures alone move to the next model. Workspace idea and expansion generation tries the next free model when a provider is unavailable, rate-limited, times out, returns malformed JSON, or fails the local schema/quality gate. It then tries the configured Groq fallback before using a grounded deterministic fallback. Idea generation must keep the combined summary and rationale between 700–1,200 characters so the Potential project or idea section is clear and usable in the public card/PDF. Expansion output remains a normal-length, implementation-ready PRD with policy context, architecture, workflow state, evidence lineage, integrations, security, observability, deployment, MVP, scale path, economics, risks, next steps, and the official source. This prevents a provider outage or shallow response from silently degrading the product. A later model may replace an invalid generation, but it cannot override a valid safety or evidence abstention in the global extraction path. No provider output can directly create a `public_signals` row.
